...
const model = {
  ...
  validateAction: function (a) {
    ...
    validateNot(
      LOCATIONS.find((l) => l.id === a.from),
      undefined,
      "Invalid from location: [" + a.from + "]"
    );
    validateNot(
      LOCATIONS.find((l) => l.id === a.to),
      undefined,
      "Invalid to location: [" + a.to + "]"
    );
      ...
      if (a.pendingDays === 0) {
        ...
        if (destination.domain && destination.domain.id === a.playerDomain) {
          destination.units = destination.units.concat(
            a.units.map((u) => ({ q: u.q, aq: u.q, type: UNIT_TYPES[u.type] }))
          );
          ...
          if (a.playerDomain === playerHouse.id) {
            ...
            actions.push(
              a.totalUnits + " " + HOUSES[a.playerDomain].name + " soldiers reached and occupied " + destination.name
            );
          }
          this.occupy(a);
          a.toRemove = true;
        ...
        if (a.playerDomain === playerHouse.id) {
          actions.push(
            "Our " + a.totalUnits + " soldiers will reach " + destination.name + " in " + a.pendingDays + " days"
          );
        } else {
